...
export class DashboardComponent implements OnInit, OnDestroy {
      ...
      tooltip: {
        ...
        callbacks: {
          label: (context) => {
            ...
            return `${value} ${this.i18nService.instant("DASHBOARD.COMMON.VEHICLE_UNIT")} (${percentage}%)`
          }
        }
      }
    }
  ...
  shouldScroll(chartData: any, threshold: number): boolean {
    if (!chartData || !chartData.datasets?.length) return false;
    return chartData.labels && chartData.labels.length > threshold;
  }
